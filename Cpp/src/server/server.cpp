...
#include "server.hpp"
#include "../common/state.hpp" // NEW: Include GlobalState
...
void Server::handle_client(int client_fd, const std::string &peer_ip)
{
    ...
    while (read_frame(client_fd, header, payload))
    {
        ...
        if (type == proto::MSG_PERM_REQUEST)
        {
            ...
            std::cout << "[server] Incoming PAIRING request from " << display << " (" << peer_ip << ")\n";
            
            // --- Wait for Web UI Approval ---
            bool accepted = GlobalState::get().waitForApproval(display + " (" + peer_ip + ")", reason);
            char ans = accepted ? 'y' : 'n';
            ...
            if (ans == 'y' || ans == 'Y')
            {
                // Issue token bound to IP to get client
        ...
        else if (type == proto::MSG_DOWNLOAD_REQ)
        {
            ...
            // C. INTERACTIVE APPROVAL (Web UI)
            std::cout << "[server] Incoming DOWNLOAD request for " << filename << " from " << peer_ip << "\n";

            // --- Wait for Web UI Approval ---
            // We pass the filename as the "Name" and the IP as the "Reason" for the UI Alert
            bool accepted = GlobalState::get().waitForApproval("Download: " + filename, "Request from " + peer_ip);
            char ans = accepted ? 'y' : 'n';
